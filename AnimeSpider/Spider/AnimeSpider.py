...
def handleUrl(text):
	...
	for tr in AnimeInfo:
		...
		AnimeDate = childrenNodes[0].string
		AnimeType = childrenNodes[1].contents[0].string
		AnimeName = childrenNodes[2].contents[0].string
		AnimeSize = childrenNodes[3].string

		# 这两个数据是js生成的，暂时不好提取
		AnimeDownload = ''
		AnimeFinish = ''

		AnimeMagnet = getManget(r'http://www.36dm.com/' + childrenNodes[2].contents[1].get('href'))
